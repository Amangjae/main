...
def _weather_bonus(restaurant: dict, weather: dict) -> tuple[float, str]:
    ...
    if category == "rainy":
        ...
        return score, "비 오는 날이라 실내 좌석과 국물 메뉴 선호를 반영했습니다."
    if category == "hot":
        score += restaurant["noodle_score"] * 0.7
        score += restaurant["indoor_score"] * 0.5
        return score, "더운 날씨라 시원한 메뉴와 실내 선호를 반영했습니다."
    if category == "cold":
...
def _history_score(restaurant: dict) -> tuple[float, str]:
    ...
    if days <= 7:
        return score - 2.5, "최근 7일 안에 방문한 기록이 있어 중복 방문을 줄이도록 감점했습니다."
    if days <= 14:
        return score - 1.0, "2주 안에 방문한 기록이 있어 소폭 감점했습니다."

    score += min(days / 10, 3.0)
    return score, "한동안 가지 않은 식당이라 재방문 후보로 올렸습니다."
...
def recommend_lunches(weather: dict | None = None) -> list[dict]:
    current_weather = weather or {"category": "clear"}
    ...
    for restaurant in restaurants:
        weather_score, weather_reason = _weather_bonus(restaurant, current_weather)
